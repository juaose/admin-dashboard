...
export async function GET(request: NextRequest) {
  try {
    ...
    // Aggregate data by SENDER'S bank (origin bank)
    const reportData = aggregateByOriginBank(allDeposits, startDate, endDate);
    ...
    console.error("Error generating origin bank report:", error);
    return NextResponse.json(
      {
        success: false,
        error: "Error al generar reporte de banco de procedencia",
...
function aggregateByOriginBank(
  deposits: any[],
  startDate: Date,
  endDate: Date
): ReportData {
  ...
  deposits.forEach((deposit) => {
    ...
    // Get SENDER'S bank name (origin bank)
    const originBankName = deposit.sendersBankName || "Desconocido";

    const customerAcc = customer.premayor_acc;

    // Initialize bank data if not exists
    if (!bankMap.has(originBankName)) {
      bankMap.set(originBankName, {
        totalAmount: 0,
        totalReloads: 0,
        customers: new Set(),
      });
    ...
    const bankData = bankMap.get(originBankName)!;
    bankData.totalAmount += amount;
    bankData.totalReloads++;
    bankData.customers.add(customerAcc);

    // Track customer data by origin bank
    const customerKey = `${originBankName}_${customerAcc}`;
